...
class VecMdpStepCollector(DataCollector):
    ...
    def collect_one_step(
        ...
        for env_idx, (
            ...
            #donkeycar
            # next_ob = next_ob.reshape((1, obs.shape[1]))
            # next_ob = self.ae.encode_from_raw_image(np.squeeze(np.array(next_ob)[:, :, ::-1]))
    ...
    def reset_after_expl(self, env_idx=None):
        self._start_new_rollout((env_idx))
    ...
    def _start_new_rollout(self, env_idx=None):
        if env_idx is None:
            ...
            #donkeycar exttra info added to obs
